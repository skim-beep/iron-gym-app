...
try:
    ...
    st.error("⚠️ ПЛАГИН НЕ УСТАНОВЛЕН. Введите в терминал: pip install streamlit-calendar")
    st.stop()
...
# --- 4. ОФИЦИАЛЬНЫЕ ШЕВРОНЫ (URL) ---
# Используем стабильные ссылки на векторные оригиналы (Wikimedia Commons / US Army Institute of Heraldry)
RANK_IMGS = {
    # SOLDIERS
    ...
    # OFFICERS (Правильные цвета: Gold/Silver)
...
def get_rank_data(xp):
    for r_min, r_max, title, abbr in FULL_RANK_SYSTEM:
        if r_min <= xp <= r_max:
            ...
            current = xp - r_min
            # Если нет картинки, ставим заглушку PV1
...
st.markdown(f"""
    ...
    /* СТИЛИЗАЦИЯ КАЛЕНДАРЯ ПОД КАМУФЛЯЖ */
