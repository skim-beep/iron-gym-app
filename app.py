...
IOS_BG = "#000000"           # True Black (OLED)
IOS_CARD = "#1C1C1E"         # Apple Dark Gray
IOS_BLUE = "#0A84FF"         # iOS Blue (Action)
IOS_GREEN = "#30D158"        # iOS Green (Success)
IOS_RED = "#FF453A"          # iOS Red (Destructive)
IOS_TEXT = "#FFFFFF"
...
st.markdown(f"""
    ...
    .ios-card {{
        ...
        border-radius: 18px; /* Apple Radius */
        padding: 20px;
        margin-bottom: 20px;
        box-shadow: 0 4px 20px rgba(0,0,0,0.4);
        border: 1px solid {IOS_BORDER};
    }}
    ...
    .progress-track {{
        width: 100%; height: 6px; background: #3a3a3c; border-radius: 10px; margin-top: 12px; overflow: hidden;
    }}
    .progress-fill {{
    ...
    /* HEADERS */
    .section-header {{
        font-size: 20px; font-weight: 700; color: white; margin: 30px 0 10px 0; letter-spacing: -0.5px;
    }}

    /* CALENDAR GRID (iOS Style) */
    ...
    div.stButton > button:active {{ transform: scale(0.95); background-color: #3a3a3c; }}
    ...
    input {{ color: white !important; font-weight: 500; }}
    div[data-baseweb="select"] > div {{ background-color: {IOS_CARD} !important; border-radius: 12px !important; border: none !important; }}
...
<div class="ios-card">
    <div class="profile-header">
        ...
        <div class="user-info">
            <h2>Sergey</h2>
            <p>{rank['title']} • {total_xp} Missions</p>
        </div>
...
# --- 7. NAVIGATION (SEGMENTED CONTROL STYLE) ---
...
if selected == "Summary":
    ...
    f_df = df.copy()
    sub_title = "Lifetime Analysis"
    if st.session_state.cal_date:
        f_df = df[df['Date'].dt.date == st.session_state.cal_date]
        sub_title = f"Filtered: {st.session_state.cal_date.strftime('%b %d, %Y')}"
        if st.button("Clear Filter", use_container_width=True):
            st.session_state.cal_date = None
            st.rerun()
            
    st.markdown(f"<div style='font-size:13px; color:#8E8E93; margin-bottom:10px; font-weight:500;'>{sub_title}</div>", unsafe_allow_html=True)
    ...
    if not f_df.empty:
        ...
        fig = go.Figure(data=go.Scatterpolar(
            ...
            line=dict(color=IOS_BLUE, width=3),
    ...
    for week in cal_obj:
        ...
        for i, day in enumerate(week):
            if day == 0:
                ...
                if is_tr: 
                    bg = IOS_GREEN # Apple Green
                    fg = "black"
                elif curr < today: 
                    # Missed days (Optional: Red dot or just gray)
                    pass 
                
                if is_tod: 
                    bg = IOS_BLUE # Apple Blue
                    fg = "white"
                ...
                st.markdown(f"""<script>
                    ...
                    for (var i = 0; i < buttons.length; i++) {{
                        if (buttons[i].innerText === "{label}") {{
                            ...
                            if ("{is_tod}" == "True") {{ buttons[i].style.fontWeight = "800"; }}
                        }}
                    }}
    ...
    st.markdown('<div class="section-header">Neural Coach</div>', unsafe_allow_html=True)
    st.markdown('<div class="ios-card">', unsafe_allow_html=True)
